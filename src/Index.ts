...
export class HoverBox {
	...
	stopListening(): void {
		if (this._listening) {
			...
			HoverBox.removeHoverListener('mouseenter', that._selector, function (this: HTMLElement) {
				that.showHoverBox(this);
			});
			HoverBox.removeHoverListener('mouseleave', that._selector, function (this: HTMLElement) {
				that.hideHoverBox(this);
			});
	...
	private getHoverBox(t: HTMLElement, showOrHide: 'show' | 'hide'): HTMLElement | undefined {
		const inlineData: HoverOptions & { hoverData?: string } = Object.assign(this._options, t.dataset);

		if (inlineData.hoverData ?? t.querySelectorAll('.HoverData').item(0)) {
			let foundHoverBox: HTMLElement = null;
			...
			if (foundHoverBox) {
				...
				if (
					...
					const idToFind = `${foundHoverBox.id}_${showOrHide === 'hide' ? 'show' : 'hide'}`,
	...
	private createHoverBox(t: HTMLElement, inlineData: HoverOptions & { hoverData?: string | undefined }): HTMLElement {
	...
	private static addHoverListener(eventType: string, delegateSelector: string, handler: (this: HTMLElement) => void) {
